...
import { getTrendingProducts } from "./api/client";
...
export default function TrendingProducts() {
    ...
    try {
      ...
      if (result.error) {
        showToast({
          ...
          title: "Failed to fetch trending products",
  ...
  const loadMore = async () => {
    ...
    try {
      const result = await getTrendingProducts(20, endCursor);
  ...
  return (
    <List isLoading={isLoading}>
      <List.Section title="Trending Products" subtitle={products.length > 0 ? `${products.length} products` : undefined}>
