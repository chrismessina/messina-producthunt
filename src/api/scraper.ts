...
export async function getFrontpageProducts(): Promise<{ products: Product[], error?: string }> {
  try {
    ...
    try {
      ...
      throw new Error(`Failed to parse Apollo data: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`);
    }
    
    // Find the homefeed data
    const homefeedEvent = apolloData.find((event) => 
      event.type === 'data' && event.result.data.homefeed
    );
    ...
    const productItems = featuredEdge.node.items.filter(
    ...
    const products = productItems.map((item) => ({
      ...
      topics: item.topics?.edges?.map((edge) => ({
        id: edge.node.id,
...
async function scrapeDetailedProductInfo(product: Product): Promise<Product> {
  try {
    ...
    if (apolloDataMatch) {
      ...
      if (sanitizedData) {
        try {
          ...
          // Find the post data
          const postEvent = apolloData.find((event) => 
            event.type === 'data' && event.result.data.post
          );
    ...
    if (!hunter) {
      ...
      const aboutHunterEl = $('#about_section > div.text-14.font-normal.text-dark-gray.text-gray-600 > div:nth-child(2) > a');
      ...
      if (!hunter) {
        ...
        if (hunterSvgEl.length > 0) {
          ...
          const hunterName = hunterEl.find('div').filter((i, el) => {
    ...
    const galleryClassElements = $('[class*="gallery" i], [id*="gallery" i], [class*="carousel" i], [id*="carousel" i]');
    ...
    if (galleryContainer.length > 0) {
      ...
      galleryClassElements.find('img').each((i, el) => {
        const imgSrc = $(el).attr('src');
        console.log(`Found gallery image from class/id selector: ${imgSrc}`);
              ...
              postData.media.forEach((mediaItem: any) => {
                if (mediaItem.url && !galleryImages.includes(mediaItem.url)) {
                  ...
                  if (!galleryImages.includes(imgUrl)) {
                    console.log(`Adding media item with imageUuid from Apollo data: ${imgUrl}`);
...
export async function scrapeOpenGraphMetadata(url: string): Promise<OpenGraphMetadata> {
  try {
    ...
    if (!metadata.image) {
      ...
      const productImage = $('.styles_thumbnail__Xtg_i img').attr('src') || 
...
export async function getProductDetails(productId: string): Promise<{ product?: Product, error?: string }> {
  try {
    ...
    const product: Product = {
      ...
      topics: postData.topics?.edges?.map((edge) => ({
        id: edge.node.id,
        name: cleanTopicName(edge.node.name),
        slug: edge.node.slug
      })) || []
...
export async function getTrendingProducts(): Promise<{ products: Product[], error?: string }> {
  try {
    ...
    // Extract product data
    const productItems = popularEdge.node.items.filter(
      (item) => item.__typename === 'Post'
    );
...
export async function getTopics(): Promise<{ topics: Topic[], error?: string }> {
  try {
    ...
    const topicsEvent = apolloData.find((event) => 
      event.type === 'data' && event.result.data.topics
    );
...
export async function searchProducts(query: string): Promise<{ products: Product[], error?: string }> {
  try {
    ...
    const searchEvent = apolloData.find((event) => 
      event.type === 'data' && event.result.data.search
    );
